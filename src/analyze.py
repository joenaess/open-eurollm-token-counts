...
def analyze_and_visualize(csv_path: str, assets_dir: str):
    ...
    df = df.with_columns(
        pl.col("part")
        .fill_null("")
        .map_elements(extract_language, return_dtype=pl.Utf8)
        .alias("language")
    )
    ...
    sns.barplot(
        data=pd_df, x="percentage", y="language", hue="language", palette="viridis"
    )
    ...
    pie_df = (
        pd_df.groupby("language", as_index=False)
        .sum()
